...
$outputDir = "C:\TerraformVersions" # Change this to your desired directory
...
foreach ($releasedVersion in $releasedVersions) {
    if (Test-Path $outputDir\$releasedVersion\terraform.exe) {
        ...
        $downloadUrl = "$baseURL/$releasedVersion/terraform_${releasedVersion}_windows_amd64.zip"
        $downloadUrl
        $response = Invoke-WebRequest -Uri $downloadUrl -Method Head -TimeoutSec 10
        if ($response.StatusCode -eq 200) {
            $zipFilePath = "$outputDir\terraform_${releasedVersion}_windows_amd64.zip"
